...
__global__ void opt_2dhistoKernel(uint32_t *input, size_t height, size_t width, uint32_t* bins){
    ...
    __shared__  unsigned int temp[HISTO_WIDTH];
/*
    if (row == 0 && col < 1024) 
        bins[col] = 0;
*/
    ...
    while(tid < INPUT_HEIGHT*INPUT_WIDTH)
    {
        atomicAdd( &temp[input[tid]], 1);
        tid += offset;
    }
    __syncthreads();

    atomicAdd( &(bins[threadIdx.x + 0]), temp[threadIdx.x + 0]);
...
/* SHARED MEMORY - THE SHITTY VERSION THAT TAKES 4 SEC
    if(row < height && col < width) {
        atomicAdd(&temp[input[col + row * ((INPUT_WIDTH + 128) & 0xffffff80)]], 1);
    }
    __syncthreads();

    bins[threadIdx.x] = temp[threadIdx.x];
    __syncthreads();
*/
    //atomicAdd(&(bins[threadIdx.x]), temp[threadIdx.x]);

    //cudaMemset(bins, 0, height * width); // Use cudamemset to set everything to zero 
    /*
...
void SetAllZero(void* bins, size_t height, size_t width)
{
    cudaMemset(bins, 0, height*width);
}
